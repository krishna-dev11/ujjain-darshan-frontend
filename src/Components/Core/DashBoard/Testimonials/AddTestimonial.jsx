...
const AddTestimonial = () => {
  ...
  const inputStyle = "w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-white placeholder-gray-700 focus:border-orange-500/30 outline-none transition-all text-sm";
  const labelStyle = "text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 ml-1 mb-2 block";
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#000] text-white p-6 md:p-12 font-sans overflow-hidden">
      ...
      <div className="relative translate-y-6 z-10 max-w-5xl mx-auto flex flex-col gap-10">
        <div className="flex flex-col gap-2">
          <nav className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-gray-500">
            <span>प्रबंधन (Admin)</span> <span className="text-orange-500">/</span>
            <span className="text-white">श्रद्धालु अनुभव (Experience Entry)</span>
          </nav>
          <h2 className="text-4xl font-bold tracking-tight">Add Spiritual Success Story</h2>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white/[0.02] border border-white/10 p-10 rounded-[3rem] backdrop-blur-2xl shadow-3xl grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-6">
            <label>
              <span className={labelStyle}>Devotee Identity (नाम)</span>
              <div className="relative group">
                <FiUser className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-orange-400" />
                <input type="text" {...register("studentName", { required: true })} className={`${inputStyle} pl-12`} placeholder="Pilgrim's full name..." />
              </div>
            </label>
            ...
            <label>
              <span className={labelStyle}>Satisfaction Rating</span>
              <div className="flex gap-3 bg-white/5 p-4 rounded-2xl border border-white/5">
                {[1, 2, 3, 4, 5].map((num) => (
                  <FiStar
                    key={num}
                    size={24}
                    className="cursor-pointer transition-all active:scale-125"
                    fill={watch("rating") >= num ? "#f97316" : "none"}
                    color={watch("rating") >= num ? "#f97316" : "#333"}
                    onClick={() => setValue("rating", num)}
                  />
                ))}
              </div>
            ...
            <label>
              <span className={labelStyle}>Experience Narrative (अनुभव विवरण)</span>
              <div className="relative group">
                <FiMessageSquare className="absolute left-4 top-5 text-gray-600" />
                <textarea {...register("message", { required: true })} rows={4} className={`${inputStyle} pl-12 resize-none`} placeholder="Describe the spiritual journey..." />
              </div>
            </label>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              <span className={labelStyle}>Visual Asset Protocol (मीडिया प्रकार)</span>
              <div className="flex bg-white/5 p-1 rounded-2xl border border-white/5">
                <button type="button" onClick={() => setIsVideo(false)} className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${!isVideo ? "bg-white text-black shadow-xl" : "text-gray-500 hover:text-white"}`}>Photo</button>
                <button type="button" onClick={() => setIsVideo(true)} className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${isVideo ? "bg-white text-black shadow-xl" : "text-gray-500 hover:text-white"}`}>Video</button>
              </div>
            </div>

            <div className="bg-white/[0.02] border border-white/5 p-6 rounded-[2rem] min-h-[300px] flex flex-col justify-center">
               <Upload name="mediaFile" label={isVideo ? "Yatra Clip" : "Devotee Photo"} register={register} setValue={setValue} errors={errors} video={isVideo} />
            </div>

            <label>
              <span className={labelStyle}>Visibility Status</span>
              <select {...register("status")} className={inputStyle}>
                <option value="Active" className="bg-black text-orange-400">Published (वेबसाइट पर दिखाएं)</option>
                <option value="Inactive" className="bg-black text-red-400">Draft (अभी न दिखाएं)</option>
              </select>
            </label>
          </div>

          <div className="lg:col-span-2 flex justify-end border-t border-white/5 pt-8">
            <button type="submit" className="px-12 py-5 bg-white text-black font-bold rounded-2xl shadow-[0_0_50px_rgba(255,165,0,0.2)] hover:bg-orange-500 hover:text-white transition-all uppercase tracking-widest text-xs active:scale-95">
              Deploy Experience
            </button>
          </div>
        </form>
