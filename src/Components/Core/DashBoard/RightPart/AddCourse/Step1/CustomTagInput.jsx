...
const CustomTagInput = ({ name, lable, register, Placeholder, errors, setValue }) => {
  ...
  // Mobile support for comma
  const handleInputChange = (e) => {
    const val = e.target.value;
    if (val.endsWith(",")) {
      const chipVal = val.slice(0, -1).trim();
      if (chipVal && !Chip.includes(chipVal)) {
        setChip([...Chip, chipVal]);
        e.target.value = "";
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        {Chip.map((c, i) => (
          <div key={i} className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-4 py-1.5 rounded-xl group animate-fadeSlideUp">
            <span className="text-[11px] font-bold text-emerald-400 uppercase tracking-wider">{c}</span>
            <button type="button" onClick={() => setChip(Chip.filter((_, idx) => idx !== i))} className="text-gray-500 hover:text-red-400 transition-colors">
              <FiX size={14} />
            </button>
          </div>
        ))}
      </div>
      {errors[name] && <p className="text-[10px] text-red-500 font-bold uppercase tracking-tighter mt-1 ml-1 animate-pulse">⚠ Protocol Error: Tags are mandatory</p>}
    </div>
  );
};
