    ...
    <div className="flex flex-col gap-10 max-w-5xl mx-auto pb-10 font-sans">
      ...
      <div className="bg-white/[0.02] border border-white/10 p-8 rounded-[2.5rem] backdrop-blur-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-10">
          ...
          <h3 className="text-xl font-bold tracking-tight text-white uppercase">Service Timeline Architect (योजना संरचना)</h3>
        </div>
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="mt-10 flex flex-col gap-4">
          <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 ml-1">
            Service Phase Name (जैसे: प्रथम दिन का कार्यक्रम) <span className="text-orange-500">*</span>
          </label>
          <div className="flex gap-3">
            <input
              placeholder="e.g. Day 1: Arrival & Mahakal VIP Darshan"
              {...register("sectionName", { required: true })}
              className="flex-1 bg-white/[0.03] border border-white/5 rounded-2xl px-6 py-4 text-white text-sm focus:border-orange-500/30 outline-none transition-all placeholder-gray-700"
            />
            <button 
              type="submit"
              className="px-8 bg-orange-500 text-white font-bold rounded-2xl hover:bg-orange-400 transition-all flex items-center gap-2 shadow-[0_0_20px_rgba(249,115,22,0.2)] active:scale-95"
            >
              <FiPlus className=" text-white " /> {editSection ? "Update Phase" : "Add Service Phase"}
            </button>
          </div>
          {editSection && (
            <button onClick={() => dispatch(SetEditSection(null))} className="text-xs text-gray-500 hover:text-white transition-colors w-fit ml-1 underline underline-offset-4">
      ...
      <div className="flex justify-between items-center px-4">
        <button 
          ...
          className="flex items-center gap-2 text-gray-500 hover:text-white transition-all text-xs font-bold uppercase tracking-widest"
        >
          <FiChevronLeft size={18} /> Service Metadata
        </button>
        <button 
          onClick={() => dispatch(setStep(3))}
          className="flex items-center gap-2 px-10 py-4 bg-white/5 border border-white/10 text-white font-bold rounded-2xl hover:bg-white/10 transition-all text-xs uppercase tracking-widest"
        >
          Publish Phase <FiChevronRight size={18} />
        </button>
      </div>
    </div>
  );
