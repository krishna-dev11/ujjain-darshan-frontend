...
import { UpdateServiceProgress } from "../../../../Services.jsx/Operations/ServicesAPI";
import toast from "react-hot-toast";

const ViewServiceMedia = () => {
  const { CourseId, SectionId, SubSectionId } = useParams();
  const { serviceDetails } = useSelector((state) => state.serviceCategory);
  const { serviceMediaUrl } = useSelector((state) => state.service);
  ...
  useEffect(() => {
    const setlectureFirstserviceMediaUrl = async () => {
      if (!serviceDetails || !serviceDetails.courseContent) return;

      const sectionIndex = serviceDetails.courseContent.findIndex(
        (section) => section._id === SectionId
      );

      if (sectionIndex === -1) return;

      const section = serviceDetails.courseContent[sectionIndex];
  ...
  const NextButton = () => {
    ...
    const subsectionIndex = serviceDetails.courseContent[sectionIndex].subSections.findIndex(
      (sub) => sub._id === SubSectionId
    );
    if (subsectionIndex === -1) return;

    const currentSection = serviceDetails.courseContent[sectionIndex];

    // last subsection of this section
    if (subsectionIndex === currentSection.subSections.length - 1) {
      if (sectionIndex < serviceDetails.courseContent.length - 1) {
        const nextSection = serviceDetails.courseContent[sectionIndex + 1];
  ...
  const PreviousButton = () => {
    ...
    if (subsectionIndex === 0) {
      if (sectionIndex > 0) {
        const prevSection = serviceDetails.courseContent[sectionIndex - 1];
      ...
      const prevSub =
        serviceDetails.courseContent[sectionIndex].subSections[subsectionIndex - 1];
      navigate(
  ...
  const IsNextVisible = () => {
    if (!serviceDetails || !serviceDetails.courseContent) return false;
    ...
    return !(
      sectionIndex === serviceDetails.courseContent.length - 1 &&
      subsectionIndex ===
        serviceDetails.courseContent[sectionIndex].subSections.length - 1
    );
  };
  ...
  const handleCompleteLecture = ()=>{
    ...
    if ( SubSectionId &&  CourseId){
         dispatch(UpdateServiceProgress(CourseId , SubSectionId , token))
    }else{
...
export default ViewServiceMedia;
