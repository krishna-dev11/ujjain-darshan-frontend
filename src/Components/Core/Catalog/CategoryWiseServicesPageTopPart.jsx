...
// const CategoryWiseServicesPageTopPart = () => {
//   const { categoryWiseServices } = useSelector(state => state.serviceCategory)
//   const categoryName = categoryWiseServices.selectedCategory.name;
...
//             <p className='text-gray-400 text-lg font-light leading-relaxed max-w-xl'>{categoryWiseServices.selectedCategory.description}</p>
//           </div>
...
//           </div>
//           <ServiceSlider data={categoryWiseServices.selectedCategory.course} />
//         </div>
...
//           </div>
//           <ServiceSlider data={categoryWiseServices.TopSellingCourses} speed={3000} />
//         </div>

//         {/* Frequent Section */}
//         <div className='flex flex-col gap-8'>
//           <h2 className='text-4xl font-bold tracking-tight px-4'>Ecosystem Bundles</h2>
//           <FrequentServices data={categoryWiseServices.RandomCategory.course} />
//         </div>
...
// export default CategoryWiseServicesPageTopPart;
...
import { useSelector } from 'react-redux'
import ServiceSlider from './ServiceSlider'
import { FiSun, FiMapPin, FiMessageCircle, FiInfo, FiArrowRight } from 'react-icons/fi'
import FrequentServices from './FrequentServices'
    ...
    <div className='relative min-h-screen bg-[#000] text-white font-sans overflow-hidden'>
      ...
      <div className='relative z-10 bg-white/[0.02] backdrop-blur-3xl border-b border-white/5 py-16 px-10'>
        <div className='max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-10'>
          <div className='flex flex-col gap-4 max-w-3xl'>
            <nav className='flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.4em] text-gray-500'>
              मुख्य पृष्ठ <span className='text-orange-500'>/</span> सेवाएं <span className='text-orange-500'>/</span> <span className='text-white'>{categoryName}</span>
            </nav>
            <h1 className='text-5xl md:text-7xl font-bold tracking-tighter text-white uppercase'>{categoryName}</h1>
            <p className='text-gray-400 text-lg font-light leading-relaxed max-w-xl'>{categoryWiseServices.selectedCategory.description}</p>
          </div>

          {/* Resources Panel Changed to Yatra Info */}
          <div className='flex flex-col gap-6 min-w-[250px] bg-white/5 p-8 rounded-[2.5rem] border border-white/10'>
            <p className='text-[10px] font-bold uppercase tracking-[0.2em] text-orange-400'>यात्रा निर्देश एवं सहायता</p>
            <div className='grid grid-cols-1 gap-3'>
              {[
                { icon: FiInfo, label: `विवरण: ${categoryName.split(" ")[0]}` },
                { icon: FiMapPin, label: "दर्शन मार्ग" },
                { icon: FiMessageCircle, label: "पंडित जी से परामर्श" },
                { icon: FiSun, label: "भस्म आरती नियम" }
              ].map((res, i) => (
                <button key={i} className='flex items-center gap-3 text-xs font-bold uppercase tracking-widest text-gray-400 hover:text-white transition-all group'>
                  <res.icon className="text-orange-500 group-hover:scale-110 transition-transform" /> {res.label}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className='relative z-10 max-w-7xl mx-auto flex flex-col gap-24 py-20 px-6'>
        ...
        <div className='flex flex-col gap-8'>
          <div className="flex items-end justify-between px-4">
             <h2 className='text-4xl font-bold tracking-tight'>सक्रिय सेवाएं (Active Services)</h2>
             <span className="text-[10px] font-bold uppercase text-gray-600 tracking-widest">Handpicked for your spiritual journey</span>
          </div>
          <ServiceSlider data={categoryWiseServices.selectedCategory.course} />
        </div>
        ...
        <div className='flex flex-col gap-8'>
           <div className="flex items-end justify-between px-4 border-l-2 border-orange-500/30 pl-8">
             <div>
                <h2 className='text-4xl font-bold tracking-tight'>लोकप्रिय पैकेज (Popular)</h2>
                <p className="text-gray-500 text-xs mt-1 uppercase tracking-widest">Most booked by devotees</p>
             </div>
             <FiArrowRight className="text-gray-700" size={32} />
          </div>
          <ServiceSlider data={categoryWiseServices.TopSellingCourses} speed={3000} />
        ...
        <div className='flex flex-col gap-8'>
          <h2 className='text-4xl font-bold tracking-tight px-4'>अन्य सुझाव (Recommendations)</h2>
          <FrequentServices data={categoryWiseServices.RandomCategory.course} />
        </div>
