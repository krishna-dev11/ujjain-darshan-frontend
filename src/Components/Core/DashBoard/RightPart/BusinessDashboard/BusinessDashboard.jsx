...
// import { getInstructorCoursesForDashboardData, getBusinessDashboardData } from "../../../../../Services.jsx/Operations/DashBoard";
// import ServiceSlider from "../../../Catalog/ServiceSlider";
// import ChartDashboard from "./BusinessProfileChart";
...
//   const { BusinessDashboardData, InstructorCoursesData, user, dashboardExtras } = useSelector((state) => state.profile);
//   const [loading, setLoading] = useState(true);

//   useEffect(() => {
//     const getDashboardData = async () => {
//       await dispatch(getBusinessDashboardData(token));
...
//   const totalAmount = BusinessDashboardData?.reduce((acc, curr) => acc + curr.totalAmountEarned, 0);
//   const totalStudents = BusinessDashboardData?.reduce((acc, curr) => acc + curr.totalStudentsEnrolled, 0);
...
//              <ChartDashboard courses={BusinessDashboardData} />
//           </div>
...
//            </div>
//            <ServiceSlider data={InstructorCoursesData} count={2} />
//         </div>
//       </div>
//     </div>
//   );
...
import ServiceSlider from "../../../Catalog/ServiceSlider";
import ChartDashboard from "./BusinessProfileChart";
import { FiSun, FiUsers, FiDollarSign, FiTrendingUp, FiActivity, FiMap } from "react-icons/fi";
...
const BusinessDashboard = () => {
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center font-mono text-orange-500 tracking-[0.5em]">SYNCING SPIRITUAL DATA...</div>;

  return (
    <div className="relative min-h-screen w-full bg-[#000] text-white p-6 md:p-12 font-sans overflow-hidden">
      <div className="absolute top-[10%] left-[-5%] select-none pointer-events-none opacity-[0.02] z-0">
        <h1 className="text-[15rem] font-bold uppercase tracking-widest leading-none">UJJAIN</h1>
      </div>
      ...
      <div className="relative z-10 translate-y-6 max-w-7xl mx-auto flex flex-col gap-10">
        ...
        {/* Header */}
        <div className="flex flex-col gap-2 border-b border-white/5 pb-8">
           <h2 className="text-4xl md:text-5xl font-bold tracking-tight">Business Intel, {user.firstName}</h2>
           <p className="text-gray-500 text-xs font-bold uppercase tracking-[0.3em] flex items-center gap-2">
             <FiActivity className="text-orange-500 animate-pulse" /> Live Yatra Metrics
           </p>
        </div>

        {/* ─── KPI BENTO GRID ─── */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard icon={FiMap} label="Total Services" value={InstructorCoursesData?.length || 0} color="bg-blue-500" />
          <StatCard icon={FiUsers} label="Total Pilgrims" value={totalStudents || 0} color="bg-orange-500" />
          <StatCard icon={FiDollarSign} label="Net Revenue" value={`₹${totalAmount || 0}`} color="bg-yellow-500" growth={dashboardExtras?.revenueGrowth} />
          <StatCard icon={FiSun} label="Active Bookings" value={dashboardExtras?.activeBatches || 0} color="bg-purple-500" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="flex flex-col gap-6">
             <div className="bg-white/[0.02] border border-white/10 p-8 rounded-[2.5rem] flex flex-col gap-4">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">Service Operations</p>
                <div className="flex justify-between border-b border-white/5 pb-4">
                   <span className="text-sm text-gray-400">Pending Inquiries</span>
                   <span className="text-sm font-bold text-orange-400">{dashboardExtras?.pendingDemos}</span>
                </div>
                <div className="flex justify-between">
                   <span className="text-sm text-gray-400">Monthly Profit</span>
                   <span className="text-sm font-bold text-orange-400">₹{dashboardExtras?.netProfit}</span>
                </div>
             </div>
             
             <div className="bg-orange-500/10 border border-orange-500/20 p-8 rounded-[2.5rem] relative overflow-hidden group">
                <FiTrendingUp className="text-orange-500 text-4xl mb-4 group-hover:scale-110 transition-transform" />
                <h4 className="text-lg font-bold text-white">Yatra Protocol</h4>
                <p className="text-xs text-orange-300/60 mt-2">Optimization of current darshan slots recommended for maximum conversions.</p>
             </div>
          </div>
        </div>
        ...
        <div className="mt-10">
           <div className="flex items-center justify-between mb-8 px-4">
              <h3 className="text-2xl font-bold tracking-tight">Active Spiritual Packages</h3>
              <button className="text-[10px] font-bold uppercase tracking-widest text-orange-400 border-b border-orange-500/30 pb-1">View Full Catalog</button>
           </div>
           <ServiceSlider data={InstructorCoursesData} count={2} />
        </div>
      </div>
