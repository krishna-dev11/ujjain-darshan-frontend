...
//           <div className="lg:col-span-2 flex flex-col gap-6">
//             <nav className="text-[10px] font-bold uppercase tracking-widest text-[#4b5563]">
//                 उज्जैन दर्शन / <span className="text-[#f97316]">{courseDetails?.category?.name || "Service"}</span>
//             </nav>
...
//               </div>
//               <div className="flex items-center gap-2 text-xs font-medium text-[#6b7280]">
//                 <FiUser className="text-[#f97316]" /> {courseDetails.studentEnrolled?.length || 0} Devotees Booked
//               </div>
//               <div className="flex items-center gap-2 text-xs font-medium text-[#6b7280]">
...
//             <div className="flex items-center gap-3 pt-6">
//                <img src={courseDetails.instructor?.imageUrl} className="w-10 h-10 rounded-full object-cover border border-[#ffffff]/10" alt="Guide" />
//                <p className="text-sm font-medium text-[#9ca3af]">
//                   Service Managed By <span className="text-[#ffffff] font-bold">{courseDetails.instructor?.firstName} {courseDetails.instructor?.lastName}</span>
//                </p>
//             </div>
//           </div>
...
//               <img src={courseDetails.thumbnail} className="w-full aspect-video rounded-2xl object-cover border border-[#ffffff]/10" alt="Package Thumbnail" />
...
//                 ) : (
//                   <button onClick={handleAddCourseInCart} className="w-full py-4 bg-[#ffffff] text-[#000000] font-bold rounded-xl text-xs uppercase tracking-widest hover:bg-[#f97316] hover:text-white transition-all">Book Service Now</button>
//                 )}
//                 <button onClick={handleShare} className="w-full py-3 border border-[#ffffff]/10 text-[#6b7280] font-bold rounded-xl text-[10px] uppercase tracking-widest hover:text-[#ffffff] transition-all flex items-center justify-center gap-2">
...
//               <div className="text-[10px] text-[#4b5563] font-bold uppercase tracking-widest flex gap-4">
//                 <span><FiLayers className="inline mr-1" /> {courseDetails.courseContent?.length} Phases</span>
...
//       const message = `Jai Mahakal! 🙏 I am interested in booking the service: *${courseDetails?.courseName}*. Please provide more details.`;
...
const ONECourseDetail = () => {
  ...
  const handleBookingAction = () => {
    ...
    if (!token) {
      ...
      const message = `Jai Mahakal! 🙏 I am interested in booking: *${courseDetails?.courseName}*. Please provide more details.`;
      const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-[#ffffff] font-sans selection:bg-[#f97316]/30">
      <Helmet>
        <title>{courseDetails?.courseName} | Ujjain Mahakal Darshan Assistance</title>
        <meta name="description" content={courseDetails?.courseDescription} />
        <link rel="canonical" href={`https://www.ujjainmahakaldarshnabooking.in/catalog/${courseDetails?.slug}`} />
      </Helmet>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            <nav className="text-[10px] font-bold uppercase tracking-widest text-gray-500">
              Ujjain Darshan / <span className="text-[#f97316]">{courseDetails?.category?.name || "Service"}</span>
            </nav>
            ...
            <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/[0.03] border border-white/5 w-fit">
               <img src={courseDetails?.instructor?.imageUrl} className="w-12 h-12 rounded-full object-cover border border-[#f97316]/30" alt="Guide" />
               <div>
                 <p className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Managed By</p>
                 <p className="text-base font-semibold">{courseDetails?.instructor?.firstName} {courseDetails?.instructor?.lastName}</p>
               </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-1 lg:sticky lg:top-24">
            <div className="bg-[#0A0A0A] border border-white/10 p-6 rounded-3xl shadow-2xl shadow-[#f97316]/5 flex flex-col gap-6">
              <div className="relative group overflow-hidden rounded-2xl">
                <img src={courseDetails?.thumbnail} className="w-full aspect-video object-cover group-hover:scale-105 transition-transform duration-500" alt="Package" />
              ...
              <div className="flex justify-between items-center">
                {/* <p className="text-4xl font-bold text-white">₹{courseDetails?.price}</p> */}
          ...
          <section className="space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-end gap-4 border-b border-white/5 pb-4">
              ...
              <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest flex gap-6">
                <span className="flex items-center gap-2"><FiLayers className="text-[#f97316]" /> {courseDetails?.courseContent?.length} Phases</span>
