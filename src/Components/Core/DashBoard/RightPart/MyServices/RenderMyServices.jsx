...
//                 {course.isOfflineBatch && (
//                   <button 
//                     onClick={() => navigate(`/BookedServices/batch-students/${course._id}`)}
...
// export default RenderMyServices;
...
import { useDispatch, useSelector } from "react-redux";
import { DeleteInstructorCourses, FetchInstructorsAllCourses } from "../../../../../Services.jsx/Operations/DashBoard";
import { FiClock, FiCheckCircle, FiEdit3, FiTrash2, FiUsers, FiCalendar, FiSun } from "react-icons/fi";
import ConfirmationModal from "../../../../Common/ConfirmationModal";
import { setService, setEditService, setStep } from "../../../../../Slices/Services";
import { useNavigate } from "react-router-dom";
import { FormateDate } from "../../../../../Utilities/FormateDate";
  ...
  return (
    <div className="flex flex-col gap-6">
      {user?.courses?.length > 0 ? (
        user.courses.map((course, index) => (
          <div key={index} className="group relative flex flex-col lg:flex-row gap-6 md:gap-8 bg-white/[0.02] border border-white/5 p-4 md:p-6 rounded-[2rem] md:rounded-[2.5rem] backdrop-blur-2xl hover:bg-white/[0.04] transition-all duration-500">
            ...
            <div className="relative w-full lg:w-72 aspect-video rounded-[1.2rem] md:rounded-[1.5rem] overflow-hidden border border-white/10 shadow-2xl">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
              <div className="absolute top-3 left-3 md:top-4 md:left-4">
                <span className={`px-2 md:px-3 py-1 rounded-full text-[8px] md:text-[10px] font-bold uppercase tracking-widest ${course.isOfflineBatch ? "bg-orange-600 text-white" : "bg-blue-500 text-white"}`}>
                  {course.isOfflineBatch ? "Live Yatra" : "Online Consult"}
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-between py-1 md:py-2">
              <div>
                <div className="flex flex-col sm:flex-row justify-between items-start gap-2 mb-3 md:mb-2">
                  <h3 className="text-xl md:text-2xl font-bold tracking-tight text-white group-hover:text-orange-400 transition-colors leading-tight">
                    {course.courseName}
                  </h3>
                  <p className="text-lg md:text-xl font-bold text-white tracking-tighter shrink-0">₹{course.price}</p>
                </div>
                <div className="flex flex-wrap gap-x-4 gap-y-2 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-gray-500">
                  <span className="flex items-center gap-1.5"><FiCalendar /> Created {FormateDate(course.createdAt)}</span>
                  <span className="flex items-center gap-1.5"><FiClock /> {course.batchTiming || "Anytime Slot"}</span>
                </div>
              </div>
              ...
              <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                  <p className="text-[8px] md:text-[9px] text-gray-600 uppercase mb-1">Visibility</p>
                  <div className={`flex items-center gap-2 text-[10px] md:text-xs font-bold ${course.status === "Draft" ? "text-pink-400" : "text-orange-400"}`}>
                    {course.status === "Draft" ? <FiClock /> : <FiCheckCircle />} {course.status === "Draft" ? "Hidden" : "Live"}
                  </div>
                </div>
                {course.isOfflineBatch && (
                  <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                    <p className="text-[8px] md:text-[9px] text-gray-600 uppercase mb-1">Bookings</p>
                    <p className={`text-[10px] md:text-xs font-bold ${course.enrollmentOpen ? "text-emerald-400" : "text-red-400"}`}>
                ...
                {course.isOfflineBatch && (
                  <button 
                    onClick={() => navigate(`/BookedServices/batch-students/${course._id}`)}
                    className="col-span-2 flex items-center justify-center gap-2 bg-orange-500/10 border border-orange-500/20 text-orange-400 rounded-2xl hover:bg-orange-500 hover:text-white transition-all text-[10px] md:text-xs font-bold uppercase tracking-widest py-3 md:py-0"
                  >
                    <FiUsers /> Devotees List
                  </button>
                )}
              </div>
            </div>
            ...
            {/* Actions Panel */}
            <div className="flex flex-row lg:flex-col gap-3 justify-center border-t lg:border-t-0 lg:border-l border-white/5 pt-5 md:pt-6 lg:pt-0 lg:pl-8">
              <button onClick={() => updateCourse(course)} className="flex-1 lg:flex-none w-full lg:w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-orange-500 hover:text-white transition-all">
                <FiEdit3 size={18} />
              </button>
              <button 
                onClick={() => setDeleteModal({
                  heading: "Confirm Removal?",
                  text1: "यह सेवा और इससे जुड़े सभी बुकिंग डेटा हमेशा के लिए हटा दिए जायेंगे।",
                  button1Text: "Delete Service", button2Text: "Cancel",
                  btn1Onclick: () => dispatch(DeleteInstructorCourses(course.instructor, course._id, token)),
                  btn2Onclick: () => setDeleteModal(null)
                })}
                className="flex-1 lg:flex-none w-full lg:w-12 h-12 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition-all"
        ...
        <div className="py-20 text-center bg-white/[0.02] border border-white/5 rounded-[2rem] md:rounded-[2.5rem] backdrop-blur-xl mx-2">
          <FiSun className="mx-auto text-gray-700 mb-4" size={40} />
          <p className="text-gray-500 font-bold uppercase tracking-widest text-[10px] md:text-xs">No active services in your catalog</p>
        </div>
      )}
