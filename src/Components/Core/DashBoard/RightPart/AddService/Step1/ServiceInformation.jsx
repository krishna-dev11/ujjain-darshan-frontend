...
// import { CreateNewCourse, EditService, GetAllCategories } from "../../../../../../Services.jsx/Operations/DashBoard";
...
//     }
//   }, [dispatch, token, editService, course, setValue]); // Course dependency yahan fix hai
...
//   errors={errors} 
//   // ─── ADD THIS LINE ───
//   viewData={editService ? course?.thumbnail : null} 
// />
//           </div>
//         </div>
...
//           <ServiceBenefitsInput name="CourseBenefits" register={register} errors={errors} setValue={setValue} getValues={getValues} />
//         </div>
//       </div>
...
//           {editService ? "Sync Updates" : "Initialize Node"}
...
const ServiceInformation = () => {
  ...
  // ================= 1. FIX: DATA PRE-FILLING LOGIC (NO LOGIC CHANGE) =================
  useEffect(() => {
    dispatch(GetAllCategories(token));
  ...
  }, [dispatch, token, editService, course, setValue]);

  const isOffline = watch("isOfflineBatch");

  // ================= 2. SUBMIT HANDLER (NO LOGIC CHANGE) =================
  const onFormSubmit = async (data) => {
    const formData = new FormData();
  ...
  const inputStyle = "w-full bg-[#ffffff]/[0.03] border border-[#ffffff]/5 rounded-2xl px-5 py-3 text-[#ffffff] placeholder-[#4b5563] focus:border-[#f97316]/30 focus:outline-none transition-all text-sm";
  const labelStyle = "text-[10px] font-bold uppercase tracking-[0.2em] text-[#6b7280] ml-1 mb-2 block";
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="flex flex-col gap-10 max-w-5xl mx-auto pb-20 animate-fadeIn">
      ...
      <div className="bg-[#ffffff]/[0.02] border border-[#ffffff]/10 p-8 rounded-[2.5rem] backdrop-blur-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-[#f97316]/5 blur-3xl pointer-events-none" />
        <div className="flex items-center gap-3 mb-8">
           <FiInfo className="text-[#f97316]" />
           <h3 className="text-xl font-bold tracking-tight text-[#ffffff] uppercase">Service Metadata (सेवा का विवरण)</h3>
        </div>
        
        <div className="grid grid-cols-1 gap-6">
          <label>
            <span className={labelStyle}>Service Title (e.g. महाकाल VIP दर्शन)</span>
            <input type="text" placeholder="Service or Ritual Name..." {...register("CourseTitle", { required: true })} className={inputStyle} />
          </label>
          <label>
            <span className={labelStyle}>Service Architecture (विस्तृत जानकारी)</span>
            <textarea rows={4} placeholder="Describe the spiritual experience or tour details..." {...register("CourseDescription", { required: true })} className={`${inputStyle} resize-none`} />
          </label>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <label>
              <span className={labelStyle}>Service Price (INR)</span>
              <input type="number" placeholder="₹ 2100" {...register("CoursePrice", { required: true })} className={inputStyle} />
            </label>
            <label>
              <span className={labelStyle}>Service Category (श्रेणी)</span>
              <select {...register("CourseCategory", { required: true })} className={`${inputStyle} appearance-none cursor-pointer`}>
                <option value="" disabled>Choose Domain</option>
                {category.map(cat => <option key={cat._id} value={cat._id} className="bg-[#0a0a0a] text-white">{cat.name}</option>)}
              </select>
      ...
      {/* SECTION 2: BATCH / SLOT CONFIGURATION */}
      <div className="bg-[#ffffff]/[0.02] border border-[#ffffff]/10 p-8 rounded-[2.5rem] backdrop-blur-2xl">
        <div className="flex items-center justify-between mb-8">
           <div className="flex items-center gap-3">
             <FiSettings className="text-[#f97316]" />
             <h3 className="text-xl font-bold tracking-tight text-[#ffffff] uppercase">Yatra / Slot Protocol</h3>
           </div>
           <label className="flex items-center gap-3 cursor-pointer group">
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#6b7280] group-hover:text-[#ffffff]">Live Booking Active</span>
              <div className="relative">
                <input type="checkbox" {...register("isOfflineBatch")} className="sr-only peer" />
                <div className="w-11 h-6 bg-[#ffffff]/10 rounded-full peer peer-checked:bg-[#f97316] transition-all after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-[#ffffff] after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full shadow-lg" />
              </div>
           </label>
        </div>
        ...
        {isOffline && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-fadeSlideUp">
            <label>
              <span className={labelStyle}><FiCalendar className="inline mr-1" /> Booking Start Date</span>
              <input type="date" {...register("batchStartDate")} className={inputStyle} />
            </label>
            <label>
              <span className={labelStyle}><FiCalendar className="inline mr-1" /> Booking End Date</span>
              <input type="date" {...register("batchEndDate")} className={inputStyle} />
            </label>
            <label>
              <span className={labelStyle}><FiClock className="inline mr-1" /> Timing Slot</span>
              <input type="text" placeholder="e.g. 04:00 AM - 08:00 AM" {...register("batchTiming")} className={inputStyle} />
            </label>
            <label>
              <span className={labelStyle}><FiUsers className="inline mr-1" /> Max Pilgrims (Capacity)</span>
              <input type="number" placeholder="10" {...register("maxSeats")} className={inputStyle} />
            </label>
            <label>
              <span className={labelStyle}>Availability Status</span>
              <select {...register("batchStatus")} className={inputStyle}>
                <option value="Upcoming">Booking Open</option>
                <option value="Ongoing">Fully Booked</option>
                <option value="Completed">Expired</option>
              </select>
            </label>
            <label className="flex items-center gap-3 mt-8">
               <input type="checkbox" {...register("enrollmentOpen")} className="w-5 h-5 rounded border-[#ffffff]/10 bg-transparent checked:bg-[#f97316]" />
               <span className="text-[10px] font-bold uppercase tracking-widest text-[#ffffff]">Enable Online Booking</span>
            </label>
          </div>
        )}
      </div>

      {/* SECTION 3: VISUALS & TAGS */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="lg:col-span-2 bg-[#ffffff]/[0.02] border border-[#ffffff]/10 p-8 rounded-[2.5rem] backdrop-blur-xl shadow-2xl relative overflow-hidden group">
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-[#f97316]/5 blur-3xl rounded-full" />
          <div className="relative z-10">
             <span className={labelStyle}>Visual Identity (Service Image)</span>
            <Upload 
              name="courseImage" 
              label="Thumbnail" 
              register={register} 
        ...
        <div className="bg-[#ffffff]/[0.02] border border-[#ffffff]/10 p-8 rounded-[2.5rem] backdrop-blur-xl">
          <span className={labelStyle}>Indexing Tags (Keywords)</span>
          <CustomTagInput name="CourseTag" register={register} errors={errors} setValue={setValue} getValues={getValues} />
        </div>

        <div className="bg-[#ffffff]/[0.02] border border-[#ffffff]/10 p-8 rounded-[2.5rem] backdrop-blur-xl">
          <span className={labelStyle}>Service Benefits (सुविधाएं)</span>
          <ServiceBenefitsInput name="CourseBenefits" register={register} errors={errors} setValue={setValue} getValues={getValues} />
        </div>
      </div>
      ...
      {/* SECTION 4: REQUIREMENTS */}
      <div className="bg-[#ffffff]/[0.02] border border-[#ffffff]/10 p-8 rounded-[2.5rem] backdrop-blur-xl">
        <CustomInstructionsInput name="CourseRequirements" label="Pilgrim Requirements (नियम और शर्तें)" register={register} errors={errors} setValue={setValue} getValues={getValues} />
      </div>

      {/* ACTION AREA */}
      <div className="flex justify-end pt-10">
        <button type="submit" className="group flex -translate-x-12 items-center gap-3 px-12 py-5 bg-[#ffffff] text-[#000000] font-bold rounded-2xl shadow-[0_0_50px_rgba(255,255,255,0.1)] hover:bg-[#f97316] transition-all uppercase tracking-[0.2em] text-[10px] active:scale-95">
          {editService ? "Sync Updates" : "Initialize Service Node"}
          <FiArrowRight className="group-hover:translate-x-1 transition-transform" />
        </button>
      </div>
    </form>
...
//         setValue={setValue}
//         errors={errors}
//         // editData={editService ? course?.thumbnail : null}
//       />
//     </div>
...
//       <button type="submit" className=" px-4 py-2 rounded-md bg-yellow-50 self-end mr-5 ">{editService ? "Save Edits" : "Next"}</button>
