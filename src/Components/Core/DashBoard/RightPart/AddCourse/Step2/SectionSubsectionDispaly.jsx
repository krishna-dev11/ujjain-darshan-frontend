...
import React, { useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { FiEdit3, FiTrash2, FiChevronDown, FiPlus, FiPlayCircle } from "react-icons/fi";
import { SetaddSubSection, SeteditSubSection, SetviewSubSection } from "../../../../../../Slices/SubSection";
import { SetEditSection, SetsectionId } from "../../../../../../Slices/Section";
import { DeleteSection, DeleteSubSection } from "../../../../../../Services.jsx/Operations/DashBoard";
import ConfirmationModal from "../../../../../Common/ConfirmationModal";
      ...
      {course?.courseContent?.map((section) => (
        <details key={section._id} className="group bg-white/[0.02] border border-white/5 rounded-3xl overflow-hidden transition-all hover:bg-white/[0.04]">
          <summary className="list-none cursor-pointer p-5 flex items-center justify-between group-open:bg-orange-500/5 transition-all">
            ...
            <div className="flex items-center gap-4 border-l border-white/5 pl-4">
              <button onClick={() => dispatch(SetEditSection(section))} className="text-gray-500 hover:text-orange-400 transition-colors">
                <FiEdit3 size={16} />
              </button>
              <button 
                onClick={() => setDeleteModal({
                  heading: "Delete Service Phase?",
                  text1: "इस फेज के अंदर की सभी एक्टिविटी हटा दी जाएगी।",
                  button1Text: "Confirm", button2Text: "Cancel",
                  btn1Onclick: () => {
          ...
          <div className="p-6 pt-2 flex flex-col gap-2">
            {section?.subSections?.map((sub) => (
              <div key={sub._id} className="flex items-center justify-between bg-white/5 border border-white/5 p-4 rounded-2xl group/sub hover:border-orange-500/30 transition-all">
                <div onClick={() => dispatch(SetviewSubSection(sub))} className="flex items-center gap-3 cursor-pointer flex-1">
            ...
            >
              <FiPlus /> New Activity Entry (जैसे: आरती समय)
            </button>
          </div>
        </details>
      ))}
