...
const BookedServiceCard = ({ data }) => {
  ...
  return (
    // Responsive width logic for 390px
    <div className="group relative flex flex-col w-full bg-[#ffffff]/[0.02] border border-[#ffffff]/5 rounded-[1.5rem] md:rounded-[2.5rem] p-3 md:p-4 transition-all duration-500 hover:bg-[#ffffff]/[0.05] shadow-xl overflow-hidden">
      
      {/* 1. Thumbnail */}
      <div 
        className="relative aspect-video rounded-[1rem] md:rounded-[2rem] overflow-hidden border border-[#ffffff]/10 cursor-pointer"
        onClick={handleStartCourse}
      >
        <img src={data.thumbnail} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="Thumbnail" />
        <div className="absolute inset-0 bg-[#000000]/40 flex items-center justify-center opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#10b981] flex items-center justify-center text-[#000000] shadow-lg">
            <FiPlay size={20} md:size={24} />
          </div>
        </div>
      </div>

      {/* 2. Course Info */}
      <div className="flex flex-col gap-2 md:gap-3 mt-4 md:mt-5 px-1 md:px-2">
        <div className="flex flex-col">
          <h3 className="text-base md:text-lg font-bold text-[#ffffff] tracking-tight truncate">
            {data.courseName}
          </h3>
          <p className="text-[8px] md:text-[10px] text-[#4b5563] font-bold uppercase tracking-widest mt-0.5">
            {data.instructor?.firstName} {data.instructor?.lastName}
          </p>
        </div>

        {/* 3. Progress Protocol */}
        <div className="mt-1 md:mt-2 space-y-1.5 md:space-y-2">
          <div className="flex justify-between text-[8px] md:text-[9px] font-bold uppercase tracking-widest text-[#4b5563]">
            <span>Sync</span>
            <span className="text-[#10b981]">60%</span>
          </div>
          <ProgressBar
        ...
        {/* 4. Footer */}
        <div className="flex justify-between items-center mt-3 md:mt-4 pt-3 md:pt-4 border-t border-[#ffffff]/5">
          <div className="flex items-center gap-2 text-[#4b5563]">
            <FiClock size={10} md:size={12} />
            <p className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest">
              <span className="text-[#10b981]">Lifetime</span>
            </p>
          </div>
          <div className="relative group/cert">
            <PiCertificateFill size={20} md:size={24} className="text-[#ffffff]/20 group-hover/cert:text-[#6A0DAD] transition-all relative z-10" />
          </div>
        </div>
      </div>
    </div>
